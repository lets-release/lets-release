  ...
  if (
    ...
    switch (pm.name) {
      ...
      default: {
        result = $(
          execaOptions,
        )`uv publish ${path.resolve(pkgRoot, distDir)} --project ${pkgRoot} --publish-url ${registry.publishUrl} ${registry.url ? ["--check-url", registry.url] : []} ${token ? ["--token", token] : ["--username", username, "--password", password!]}`;
        break;
      }
    }
