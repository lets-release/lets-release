...
**[lets-release][]** plugin to analyze commits with [conventional-changelog][]
...
**Note**: Individual properties of `parserOptions` will override ones loaded with an explicitly set `preset` or `config`. If `preset` or `config` are not set, only the properties set in `parserOptions` will be used.
...
The properties to set in the rules will depends on the commit style chosen. For example [conventional-changelog-angular][angular] use the commit properties `type`, `scope` and `subject` but [conventional-changelog-eslint][eslint] uses `tag` and `message`.
