...
export class InvalidMaintenanceMergeError extends LetsReleaseError {
  ...
  get details() {
    return `Only releases within the range \`${getRange(this.range.mergeMin, this.range.mergeMax)}\` of package \`${this.pkg.uniqueName}\` can be merged into the maintenance branch \`${this.branch}\` and published to the \`${JSON.stringify(this.release.channels)}\` distribution channels.
